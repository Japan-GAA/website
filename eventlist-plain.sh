...
const place = (e: Ev) => (ja ? e.locationJa : e.location);
---
      ...
      <div class="event__body">
        ...
        {place(e) && (
          <p class="event__where">
            {isUrl(place(e))
              ? <a href={place(e)} target="_blank" rel="noopener">{ja ? "地図を見る" : "See the map"}</a>
              : place(e)}
          </p>
        )}
        {!past && (ja ? e.notesJa : e.notes) && (
          <p class="event__notes">{ja ? e.notesJa : e.notes}</p>
        )}
      </div>
    </li>
  ))}
